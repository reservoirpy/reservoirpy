...
use_raw_input = (
    True  # use input directly (alongside internal states) when computing output
)
...
proba_non_zero_connec_W = (
    0.2  # Sparsity of recurrent matrix: Perceptage of non-zero connections in W matrix
)
proba_non_zero_connec_Win = 1.0  # Sparsity of input matrix
proba_non_zero_connec_Wfb = 1.0  # Sparsity of feedback matrix
alpha_coef = (
    1e-8  # to init the inverse of state correlation matrix used in FORCE learning
)
...
# Train reservoir
internal_trained = reservoir.train(
    inputs=[train_in], teachers=[train_out], wash_nr_time_step=initLen, verbose=False
)
...
mse = np.mean(
    (test_out[:] - output_pred[0]) ** 2
)  # Mean Squared Error: see https://en.wikipedia.org/wiki/Mean_squared_error
rmse = np.sqrt(
    mse
)  # Root Mean Squared Error: see https://en.wikipedia.org/wiki/Root-mean-square_deviation for more info
nmrse_mean = abs(rmse / np.mean(test_out[:]))  # Normalised RMSE (based on mean)
nmrse_maxmin = rmse / abs(
    np.max(test_out[:]) - np.min(test_out[:])
...
plt.ylim([-1.1, 1.1])
plt.title(
    "Activations $\mathbf{x}(n)$ from Reservoir Neurons ID 0 to 11 for 200 time steps"
)
