...
def check_timeseries(x, *, expected_dim=None, expected_length=None):
    ...
    if expected_dim is not None and not x.shape[1] == expected_dim:
        raise ValueError(f"Expected feature dimension to be {expected_dim}, got {x.shape[1]}.")
...
def check_multiseries(x, *, expected_dim=None, expected_length=None):
    if isinstance(x, Sequence):
        for ts in x:
            check_timeseries(ts, expected_dim=expected_dim, expected_length=expected_length)
        ...
        if expected_dim is not None and not x.shape[2] == expected_dim:
            raise ValueError(f"Expected feature dimension to be {expected_dim}, got {x.shape[2]}.")
    else:
        raise TypeError(f"Expected a 3-dimensional array or a sequence of array, got {type(x)}.")
...
def check_node_input(x, *, expected_dim=None, expected_length=None):
    if isinstance(x, Sequence):
        for ts in x:
            check_timeseries(ts, expected_dim=expected_dim, expected_length=expected_length)
            if expected_dim is None:  # Ensure all series have the same number of features
                expected_dim = ts.shape[1]
    elif is_array(x):
        if x.ndim == 3:
            check_multiseries(x, expected_dim=expected_dim, expected_length=expected_length)
        elif x.ndim == 2:
...
def check_model_timestep(x, *, expected_inputs=None, expected_dim=None):
    if isinstance(x, Mapping):
        if expected_inputs is not None and set(expected_inputs) != set(x.keys()):
            raise ValueError(f"Input does not match expected node names: {set(expected_inputs)} != {set(x.keys())}")
...
def check_model_input(x, *, expected_dim=None, expected_length=None):
    if isinstance(x, Mapping):
        for name in x:
            ...
            check_node_input(x[name], expected_dim=node_dim, expected_length=expected_length)
    elif is_array(x) or isinstance(x, Sequence):
        if isinstance(expected_dim, Mapping):
            if len(expected_dim) == 1:
                expected_dim = next(iter(expected_dim.values()))
            else:
                raise TypeError(f"Expected a mapping of node inputs ({list(expected_dim.keys())}), but got {type(x)}.")
