...
def test_generate_internal_sparse(sr, proba):
    ...
    if sparse.issparse(W):
        assert_almost_equal(
            np.sum(W.toarray() != 0.0) / W.toarray().size, proba, decimal=1
        )
    else:
        assert_almost_equal(np.sum(W != 0.0) / W.size, proba, decimal=1)
...
def test_reproducibility_W():
    ...
    W0 = generate_internal_weights(
        N=100, sr=1.2, proba=0.4, dist="uniform", low=-1, high=1, seed=seed0
    ).toarray()

    seed1 = default_rng(78946312)
    W1 = generate_internal_weights(
        N=100, sr=1.2, proba=0.4, dist="uniform", low=-1, high=1, seed=seed1
    ).toarray()

    seed2 = default_rng(6135435)
    W2 = generate_internal_weights(
        N=100, sr=1.2, proba=0.4, dist="uniform", low=-1, high=1, seed=seed2
    ).toarray()
